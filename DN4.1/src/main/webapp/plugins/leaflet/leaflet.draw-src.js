...
*/
define(["./leaflet-src","./leaflet.css","./leaflet.draw.css"], function(){ // add define code by qinjunwen for test
    (function (window, document, undefined) {/*
        ...
        L.drawLocal = {
            draw: {
                toolbar: {
                    actions: {
                        title: locale.get('cancel'),//'取消',//'Cancel drawing',
                        text: locale.get('cancel')//'取消'
                    },
                    buttons: {
                        polyline: 'Draw a polyline',
                        polygon: 'Draw a polygon',
                        rectangle: locale.get("iwos_circle_to_show_markers_name"),//'Draw a rectangle',
                        circle: 'Draw a circle',
                        marker: locale.get('addsite')//'添加监测点'
                    }
                },
                handlers: {
                    circle: {
                    ...
                    marker: {
                        tooltip: {
                            start: locale.get("click_to_add_marker")//'点击地图上的位置'
                        }
                    },
                    polygon: {
                        tooltip: {
                    ...
                    rectangle: {
                        tooltip: {
                            start: locale.get("iwos_circle_to_show_markers_name")//'框选图标可显示名称.'
                        }
                    },
                    simpleshape: {
                        tooltip: {
            ...
            edit: {
                toolbar: {
                    actions: {
                        save: {
                            title: locale.get('save1'),//'保存',
                            text: locale.get('save1')//'保存'
                        },
                        cancel: {
                            title: locale.get("cancel"),//'取消',
                            text: locale.get('cancel')//'取消'
                        }
                    },
                    buttons: {
                        edit: locale.get("edit_location"),
                        editDisabled: '没有监测点.',
                        remove: locale.get("deletesite"),
                        removeDisabled: '没有监测点.'
                    }
                },
                handlers: {
                    edit: {
                        tooltip: {
                            text: locale.get("drag_to_change_the_location"),//'拖动图标以编辑监测点位置.',
                            subtext: locale.get("cancel_editing_rollback")//'点击取消以回退操作.'
                        }
                    },
                    remove: {
                        tooltip: {
                            text: locale.get("click_site_to_delete")//'点击要删除的监测点'
                        }
                    }
                }
            }
        ...
        L.EditToolbar.Edit = L.Handler.extend({
            ...
            _revertLayer: function (layer) {
                ...
                if (this._uneditedLayerProps.hasOwnProperty(id)) {
                    ...
                    // add by qinjunwen 2014/3/19 in IWOS 
                    this._map.fire("draw:editrevert",{
                        layer : layer,
                        uneditedProps : this._uneditedLayerProps[id]
                    });
                }
            },
