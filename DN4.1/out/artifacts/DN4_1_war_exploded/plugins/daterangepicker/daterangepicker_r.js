  ...
  // Finally, as a browser global.
    ...
    DateRangePicker.prototype = {
        ...
        setOptions: function(options, callback) {
            ...
            this.startDate = moment().startOf('day'); //初始选择的日期范围
            this.endDate = moment().endOf('day');     //
            this.timeZone = moment().utcOffset();
            this.minDate = false;                    //用户可以选择的最早日期
            this.maxDate = false;                    //用户可以选择的最晚日期
            this.dateLimit = false;                  //开始和结束日期最大间隔跨度。可已添加属性到当前时刻(年：天：时：分：...)

            this.showDropdowns = false;             //在日历上显示跳到指定年月的下拉选择框
            this.showWeekNumbers = false;           //在日历上显示week
            this.timePicker = false;                //允许选择小时分钟。。
            this.timePickerSeconds = false;         //在timePicker上允许选择 秒数
            this.timePickerIncrement = 30;          //时间的增量，单位为分钟
            this.timePicker12Hour = true;          //是否使用12小时制来显示时间
            this.singleDatePicker = false;         //只显示一个日期选择，这样开始日期和结束日期相同，相当于单一日期选择
            this.ranges = {};

            this.opens = 'right';                   //(string: 'left'/'right'/'center')日期选择框的弹出位置
            if (this.element.hasClass('pull-right'))
                this.opens = 'left';

            this.drops = 'down';                    //(string: 'down' or 'up')
